...
App.Store = (function () {
  ...
  function load() {
    ...
    s.categories.forEach(function (c) {
      if (c.id === 'c-cook' || c.name.toLowerCase().indexOf('cook') > -1) c.icon = 'flame';
      else if (c.id === 'c-serve' || c.name.toLowerCase().indexOf('serv') > -1) c.icon = 'plate';
      else if (c.id === 'c-util' || c.name.toLowerCase().indexOf('uten') > -1) c.icon = 'utensils';
      else if (c.id === 'c-furn' || c.name.toLowerCase().indexOf('furn') > -1) c.icon = 'chair';
      else if (c.id === 'c-trans' || c.name.toLowerCase().indexOf('trans') > -1) c.icon = 'truck';
      else if (c.id === 'c-styl' || c.name.toLowerCase().indexOf('styl') > -1) c.icon = 'sparkles';
  ...
  function searchItems(q, catId, sortBy, stockFilter) {
    ...
    var filtered = s.items.filter(function (i) {
      ...
      // Stock health filter (All, Low, Empty, In Stock)
  ...
  function itemStats(id) {
    ...
    return {
      ...
      inKitchen: inInventory, // Backward-compat alias
  ...
  /* Item Usage Trend Data for Visual Graphs */
  ...
  /* Overall Recovery Trend across recent events */
  ...
  /* Close event with dedicated consumable deduction and optional durable write-off */
  ...
  /* Export Event Manifest to Formatted Plain Text (for Notepad, SMS, WhatsApp) */
